...
const uploadAzureBlob = async (containerName, blobName, inputFilePath) => {
  ...
  try {
    ...
    console.log(
      `File Uploaded to https://${process.env.AZURE_STORAGE_ACCOUNT}.blob.core.windows.net/${containerName}/${blobName}`
    );
  } catch (err) {
...
const uploadAzureBlobFolder = async (containerName, prefix, inputDirPath) => {
  try {
    ...
    console.log(
      `Folder uploaded to https://${process.env.AZURE_STORAGE_ACCOUNT}.blob.core.windows.net/${containerName}/${prefix}`
    );
  } catch (err) {
